...
const migrationPath = path.resolve(process.cwd(), 'supabase/migrations/20260802000001_add_shipping_info_and_storage_tips_to_products.sql')
...
async function run() {
  ...
  const { data, error } = await supabase.from('products').select('id, shipping_info, storage_tips').limit(1)
  if (error) {
    console.error('Column test error:', error.message)
  } else {
    console.log('✅ shipping_info and storage_tips columns verified on products table!', data)
  }
}
