...
async function debugAll() {
      ...
      for (const user of users) {
        for (const pass of passes) {
          try {
            ...
            console.log(`CONNECTED ${host}:${port} user=${user}:`, dbRes.rows[0])
            ...
            `)
            console.log(`Products columns on ${host}:${port}:`, colRes.rows)
            ...
            `)
            console.log(`Successfully altered public.products on ${host}:${port}`)
  ...
  // Delay 2 seconds for PostgREST cache reload
  ...
  console.log('Result via Supabase JS Client:', JSON.stringify(res, null, 2))
}
